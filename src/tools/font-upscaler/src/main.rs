...
fn main() {
	...
	while let Some(arg) = args.next() {
		match arg.as_str() {
			"-i" | "--input-dir" => dirs.push(
				args.next()
					.expect("unexpected end of args (expected input dir)"),
			),
			"-e" | "--exts" => exts.push(
				args.next()
					.expect("unexpected end of args (expected file ext)"),
			),
			"-o" | "--output-dir" => {
				output_dir = args
					.next()
					.expect("unexpected end of args (expected output dir)")
			}
			"-h" | "--height" => {
				cell_height = args
					.next()
	...
	multithread(dirs, None, move |_tn, (in_dir, out_dir, exts)| {
		...
		let _processed = multithread(files, None, move |_tn2, (file, in_dir, out_dir)| {
			...
			let upscaled_img = image::ImageBuffer::<Rgba<u8>, _>::from_vec(
				width as u32 * 4,
				height as u32 * 4,
				upscaled_cells,
			)
	...
	fn is_present_with_overflow(&self, x: isize, y: isize) -> bool {
		let res = if x < 0 || y < 0 || x >= self.width as _ || y >= self.height as _ {
			...
			match pixel.2 .0[3] {
				...
				u8::MAX => {
					/* println!("({x}, {y}): true, h{} w{}", self.height, self.width); */
					true
				}
				_ => {
